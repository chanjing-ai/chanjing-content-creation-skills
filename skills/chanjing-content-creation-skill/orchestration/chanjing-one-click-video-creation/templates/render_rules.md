...
**`ref_prompt` 的装配、题材路由、D.1–D.4、自检问句**以技能包 **`templates/storyboard_prompt.md`**（当代与 **D.1b** 共用真值）及 **`templates/history_storyboard_prompt.md`**（非当代）为准；**`README.md` §4.2** 为指针，本文不重复。
...
4. **Render**  
   - 须满足本文 **§3 技术规则**与 **§4 硬性约束**；`ref_prompt` 内容与质检须同时满足 **`storyboard_prompt.md`** / **`history_storyboard_prompt.md`**（见 **`README.md` §4.2** 指针）。  
...
- **公共库须先全量再全局匹配**：执行 **`list_figures --source common --fetch-all --page-size 80 --json`**（或等价地翻页直至当前条件下的 `total_count` 全覆盖）。**禁止**在未合并全库前只从第一页或前几页挑人；**禁止**未在全量候选上比较就取接口返回顺序最前几项。
- **标签字典（更精准）**：可先 **`list_tag_dict --business-type 1 --json`** 查看大类（`name` / `business_type` / `weight` / `update_time` / `tag_list`）与子标签（`id` / `parent_id` / `level` / `weight` 等），再 **`list_figures --source common --tag-ids <子标签 id 逗号分隔> --fetch-all …`**；`tag_ids` 多值为 **AND**，仍须在筛选结果上 **`--fetch-all` 覆盖 `total_count`**。细则见 **`chanjing-video-compose_SKILL.md`** / **`reference.md`**。
- **在完整列表上择优**：结合当次 **`video_plan`（含 `tone`）**、选题、各镜口播气质与场景（`name`、`tag_names`、`audio_name` 语义，以及 `figures[]` 朝向/分辨率），先筛出与内容**较相符的一批候选项**（通常 3～8 个），再比较 **`figures[].width`/`height`**、TTS 与 **`audio_man_id`** 一致性，最终确定 **`person_id` + `figure_type`**。**默认偏好年轻形象**（名称或 `audio_name` 体现青年/元气/学生等；**仅当** Plan/口播明确要求成熟、权威、中老年、历史叙事等时再优先匹配对应气质）；与 **`README.md` §4**（列表与匹配流程）及当次 **`video_plan`** 对齐。
...
| 3 | **镜头**：第 1、最后一镜数字人；中间奇数数字人、偶数 AI（规程见 **`storyboard_prompt.md`**「分镜结构与切段」；无自动化时 Agent 自检）。**首个数字人分镜**（按 `scene_id` 排序后首条 `use_avatar=true`）：口播 **≤20 字**，目标成片 **3–5 秒**；`run_render.py` **校验字数**，时长以 TTS 切段为准，过长口播须在内容层压缩。 |
