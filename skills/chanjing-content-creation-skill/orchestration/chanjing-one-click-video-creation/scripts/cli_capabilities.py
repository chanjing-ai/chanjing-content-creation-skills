...
def list():
    return capability_catalog(
        SKILL_NAME,
        manual="chanjing-one-click-video-creation_SKILL.md",
        operations=[
...
def config():
    return config_contract(
        preconditions=["先通过 chanjing-credentials-guard", "本机存在 ffmpeg 与 ffprobe"],
...
def usage():
    return usage_contract(
        examples=[
            "python skills/chanjing-content-creation-skill/orchestration/chanjing-one-click-video-creation/scripts/run_render.py --input workflow.json --output-dir ./outputs/run1",
...
def run_render(input_file: str, output_dir: str):
    return run_skill_script(
        SKILL_NAME,
        "run_render.py",
        args=["--input", input_file, "--output-dir", output_dir],
    )
