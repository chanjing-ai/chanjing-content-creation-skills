...
脚本；不调用 list_tasks。渲染规则以同技能包 templates/render_rules.md 为准；ref_prompt 见 templates/storyboard_prompt.md 与 history_storyboard_prompt.md（README.md §4.2 指针）；分镜字段见 storyboard_prompt.md；字段契约见 README.md §6 与 examples/workflow-contract.md。首个数字人分镜 voiceover 默认≤20 字（环境变量 FIRST_DIGITAL_HUMAN_MAX_CHARS 可调）。
"""
...
# 分镜连续合并为 TTS 批时的字符上限（低于接口 ~4000 字）；见 render_rules.md §3·C.4；编排见 README.md §5（Agent 通用模式）
TTS_BATCH_MAX = 3900
