...
def list():
    return capability_catalog(
        SKILL_NAME,
        manual="chanjing-tts_SKILL.md",
        operations=[
            {"name": "list_voices", "script": "list_voices"},
...
def config():
    return config_contract(
        preconditions=["先通过 chanjing-credentials-guard", "使用 UTF-8 文本"],
...
def usage():
    return usage_contract(
        examples=[
            "python skills/chanjing-content-creation-skill/products/chanjing-tts/scripts/list_voices --json",
            "python skills/chanjing-content-creation-skill/products/chanjing-tts/scripts/create_task --audio-man <id> --text '你好'",
            "python skills/chanjing-content-creation-skill/products/chanjing-tts/scripts/poll_task --task-id <id>",
        ],
...
def list_voices(json_output: bool = False):
    ...
    return run_skill_script(SKILL_NAME, "list_voices", args=args)
