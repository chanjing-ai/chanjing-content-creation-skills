...
脚本：`scripts/list_tag_dict`（`--business-type 1` 常见为数字人；`--json` 输出完整结构）。
...
| 脚本 | 对应接口 |
|------|----------|
| `list_tag_dict` | `GET /open/v1/common/tag_list`；`--business-type 1` 常见为数字人；大类与子标签字段见上文「标签字典」 |
| `list_figures` | `GET /open/v1/list_common_dp` 或 `POST /open/v1/list_customised_person`；公共库可选 **`--tag-ids`**（AND）、**`--common-dp-source`**；**`--fetch-all`** 拉全量至当前条件下 `total_count`；或 **`--max-pages`** 多页合并；建议 `--json` 与较大 `--page-size` |
