...
def list():
    return capability_catalog(
        SKILL_NAME,
        manual="chanjing-video-compose_SKILL.md",
        operations=[
            {"name": "list_figures", "script": "list_figures"},
            {"name": "list_tag_dict", "script": "list_tag_dict"},
            {"name": "upload_file", "script": "upload_file"},
            {"name": "create_task", "script": "create_task"},
            {"name": "poll_task", "script": "poll_task"},
            {"name": "download_result", "script": "download_result"},
        ],
    )
...
def config():
    return config_contract(
        preconditions=["先通过 chanjing-credentials-guard", "创建任务前明确字幕 show/hide"],
...
def usage():
    return usage_contract(
        examples=[
            "python skills/chanjing-content-creation-skill/products/chanjing-video-compose/scripts/list_tag_dict --business-type 1 --json",
            ...
            "python skills/chanjing-content-creation-skill/products/chanjing-video-compose/scripts/list_figures --source common --tag-ids 23,41 --fetch-all --page-size 80 --json",
            "python skills/chanjing-content-creation-skill/products/chanjing-video-compose/scripts/create_task --person-id <id> --audio-man <voice_id> --subtitle hide --text '你好'",
...
def list_figures(source: str, json_output: bool = False, page_size: int | None = None, max_pages: int | None = None):
    ...
    if json_output:
        args.append("--json")
    return run_skill_script(SKILL_NAME, "list_figures", args=args)
...
def poll_task(video_id: str):
    return run_skill_script(SKILL_NAME, "poll_task", args=["--id", video_id])
