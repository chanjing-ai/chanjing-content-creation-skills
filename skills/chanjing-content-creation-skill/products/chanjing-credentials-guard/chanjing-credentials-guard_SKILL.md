...
- **本文件（`chanjing-credentials-guard_SKILL.md`）**：业务逻辑、追问与异常语义；据此调度 [`scripts/cli_capabilities.py`](scripts/cli_capabilities.py) 与 [`scripts/`](scripts/) 下脚本。
...
2. **Offer two setup options** after the user obtains keys:
   ...
   - Option B: let the user paste `AK=... SK=...` in chat and apply it for them by running `python skills/chanjing-content-creation-skill/products/chanjing-credentials-guard/scripts/chanjing-config --ak ... --sk ...`.
...
To open the login page manually: `python skills/chanjing-content-creation-skill/products/chanjing-credentials-guard/scripts/open_login_page`
...
Check if local AK/SK already exists (read `~/.chanjing/credentials.json` for non-empty `app_id` and `secret_key`, or run `python skills/chanjing-content-creation-skill/products/chanjing-credentials-guard/scripts/chanjing-config --status`).
...
You can run `python skills/chanjing-content-creation-skill/products/chanjing-credentials-guard/scripts/open_login_page` first to open the login page, then either run the config command yourself or paste `AK=... SK=...` in the conversation for the agent to apply.
...
**Shortcut**: Run `python skills/chanjing-content-creation-skill/products/chanjing-credentials-guard/scripts/chanjing-get-token`; on success it prints access_token, on failure it prints guidance.
...
# Set AK/SK manually
python skills/chanjing-content-creation-skill/products/chanjing-credentials-guard/scripts/chanjing-config --ak <app_id> --sk <secret_key>
