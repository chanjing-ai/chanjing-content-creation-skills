...
- **本文件（`chanjing-avatar_SKILL.md`）**：业务逻辑、追问与异常语义；据此调度 [`scripts/cli_capabilities.py`](scripts/cli_capabilities.py) 与 [`scripts/`](scripts/) 下脚本。
...
### 对口型 `model` 参数（API 请求体 / `create_task --model`）

| 取值 | 说明 |
|------|------|
| **0**（默认） | 默认基础版：**蝉镜 lip-sync 模型** |
| **1** | 高质版：**蝉镜 lip-sync Pro 模型** |
| **2** | 卡通形象专用：**训练 / 驱动素材必须为卡通形象**，否则效果与合规风险由使用方承担 |

CLI 示例：`create_task ... --model 1`；JSON 请求体字段 `"model": 0` 与上表一致。
...
本 Skill 提供脚本（`skills/chanjing-content-creation-skill/products/chanjing-avatar/scripts/`），与 **chanjing-credentials-guard** 使用同一配置文件（`~/.chanjing/credentials.json`）获取 Token。

| 脚本 | 说明 |
|------|------|
| `get_upload_url` | 获取上传链接，输出 `sign_url`、`mime_type`、`file_id` |
| `upload_file` | 上传本地文件，轮询 file_detail 直到就绪后输出 `file_id` |
| `create_task` | 创建对口型任务（TTS 或音频驱动），支持 `--model`：0 基础 lip-sync、1 lip-sync Pro、2 卡通专用（素材须卡通）；输出视频任务 id |
...
VIDEO_FILE_ID=$(python skills/chanjing-content-creation-skill/products/chanjing-avatar/scripts/upload_file --service lip_sync_video --file ./my_video.mp4)

# 2. 创建 TTS 对口型任务（需先通过 list_common_audio 获取 audio_man_id）
TASK_ID=$(python skills/chanjing-content-creation-skill/products/chanjing-avatar/scripts/create_task \
  --video-file-id "$VIDEO_FILE_ID" \
  --text "君不见黄河之水天上来" \
  --audio-man-id "C-f2429d07554749839849497589199916" \
  --model 0)
# --model：0 基础 lip-sync（默认可省略）；1 Pro 高质；2 卡通专用（素材须卡通）
